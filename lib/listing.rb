...
class Listing
  ...
  def self.all
    ...
    data.map do |listing|
      Listing.new(id: listing['id'], title: listing['title'], price: listing['price'], description: listing['description'],
  ...
  def self.find(id:)
    data = DatabaseConnection.query("SELECT * FROM listings
      WHERE id = '#{id}';")
    Listing.new(id: id, title: data[0]['title'], price: data[0]['price'], description: data[0]['description'],
  ...
  def self.add(title:, price:, description:, host_id:)
    ...
    Listing.new(id: data[0]['id'], title: data[0]['title'], price: data[0]['price'],
                description: data[0]['description'], host_id: data[0]['host_id'])
  end
end
